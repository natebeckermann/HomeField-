...
import {Search,Settings2,ChevronRight,CalendarDays,Newspaper,Trophy,Home as HomeIcon,Radio} from "lucide-react";
...
const KNOWN_BADGES:Record<string,string>={
 "stl-cardinals":"https://a.espncdn.com/i/teamlogos/mlb/500/stl.png",
 "stl-blues":"https://a.espncdn.com/i/teamlogos/nhl/500/stl.png",
 "indiana-football":"https://a.espncdn.com/i/teamlogos/ncaa/500/84.png",
 "indiana-basketball":"https://a.espncdn.com/i/teamlogos/ncaa/500/84.png",
 "slu-basketball":"https://a.espncdn.com/i/teamlogos/ncaa/500/139.png",
 "stl-city":"https://a.espncdn.com/i/teamlogos/soccer/500/21812.png",
...
export default function HomeField(){
 ...
 useEffect(()=>{let cancelled=false;async function load(){setGamesLoading(true);const results=await Promise.all(teams.map(async team=>{try{const res=await fetch(`/api/sports/team?key=${encodeURIComponent(team.id)}&name=${encodeURIComponent(API_NAMES[team.id]||team.name)}`);if(!res.ok)throw new Error("sports feed unavailable");const data=await res.json();const badge=KNOWN_BADGES[team.id]||data?.team?.badge;if(data?.nextEvent)return {teamId:team.id,title:data.nextEvent.name||team.name,detail:formatEventDetail(data.nextEvent),status:data.nextEvent.status||"NEXT",eventId:data.nextEvent.id,isLiveData:true,badge,source:data.source} satisfies LiveCard;if(data?.team)return {teamId:team.id,title:team.name,detail:"Schedule unavailable",status:"TEAM",isLiveData:true,badge,source:data.source} satisfies LiveCard}catch{}const fallback=events.find(e=>e.teamId===team.id);return fallback?{teamId:team.id,title:fallback.title,detail:fallback.detail,status:fallback.status,isLiveData:false,badge:KNOWN_BADGES[team.id]} satisfies LiveCard:null}));if(!cancelled){setLiveCards(results.filter(Boolean) as LiveCard[]);setGamesLoading(false)}}if(teams.length)load();else{setLiveCards([]);setGamesLoading(false)}return()=>{cancelled=true}},[teams]);
...
function TeamBadge({src,label,small=false}:{src:string;label:string;small?:boolean}){const [failed,setFailed]=useState(false);if(failed)return <div className={small?"gameBadge teamBadgeFallback":"teamLogo teamBadgeFallback"}>{label}</div>;return <img src={src} alt="" className={small?"gameBadge":"teamLogoImage"} onError={()=>setFailed(true)}/>}
